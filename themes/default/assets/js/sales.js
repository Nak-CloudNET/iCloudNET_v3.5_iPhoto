...
if (slwarehouse = __getItem('slwarehouse')) {
	...
	$(document).on('change', '#poption', function () {
        ...
        var default_price = $('#pgroup_price').val();
			...
			$.each(item.all_group_prices, function () {
				...
				if(opt ==0){
                    if(item.options !== false) {
                        ...
                        if (item.customer_percent == 0) {
                            price = default_price;
                        } else {
                            price = (default_price + ((default_price * item.customer_percent) / 100));
                        }

                        $('#pprice,#pprice_show').val(parseFloat(price));
                        $("#net_price").text(formatMoney(price));
                        $('#poption').trigger('change');
                    }else {
	...
	if (__getItem('slitems')) {
		...
		$.each(slitems, function () {
			...
			tr_html += '<td class="text-right">' +
				...
				'<input type="hidden" name="qty_rec[]" class="qty_rec" id="qty_rec_' + row_no + '" value="' + qty_rec + '" />' +
            ...
            if (delivery_id) {
                tr_html += '<td class="text-center"><i class="fa fa-times tip pointer" id="' + row_no + '" title="Remove" style="cursor:pointer;" ids="' + product_id + '" qty="' + item_qty + '"></i></td>';
            } else {
