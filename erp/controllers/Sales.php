...
class Sales extends MY_Controller
{
	...
	function customer_balance()
	{
        ...
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => site_url('sales'), 'page' => lang('sales')), array('link' => '#', 'page' => lang('customer_balance_list')));
    ...
    function add($sale_order_id = NULL, $delivery_id = NULL,$quote_ID = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			if($invoice_view == 0){
                redirect("sales/print_iphoto_invoice/" . $s->row()->id);
			}
...
//            $this->erp->print_arrays($data, $products, $payment);
     ...
     function invoice_camera_city($id=null)
    {
        ...
        $this->data['title'] = "2";
        $this->data['sid'] = $id;
        $this->load->view($this->theme .'sales/invoice_camera_city',$this->data);
    }
